...
int Slicer3_main(int argc, char *argv[])
{
...
#ifdef USE_PYTHON
  std::cerr << "Starting to InitializePython" << std::endl;
  Py_Initialize();
  ...
  if (PythonModule == NULL)
    {
    slicerCerr ( "Warning: Failed to initialize python" << endl );
    return 1;
    }
  ...
  if (v == NULL)
    {
    PyErr_Print();
    slicerCerr ( "Error: Failed to initialize Python" << endl );
    return 1;
    }
  ...
  if ( (long)interp == -1 )
    {
    std::cerr << "Error: Failed to get Tcl interp address from Python" << endl;
    return 1;
    }
  Py_DECREF(v);
  if (Py_FlushLine())
