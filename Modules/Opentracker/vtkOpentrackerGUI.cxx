...
//---------------------------------------------------------------------------
vtkStandardNewMacro (vtkOpentrackerGUI );
vtkCxxRevisionMacro ( vtkOpentrackerGUI, "$Revision: 1.0 $");


//---------------------------------------------------------------------------
vtkOpentrackerGUI::vtkOpentrackerGUI ( )
{
    this->Logic = NULL;
...
//---------------------------------------------------------------------------
void vtkOpentrackerGUI::PrintSelf ( ostream& os, vtkIndent indent )
{
    this->vtkObject::PrintSelf ( os, indent );

    os << indent << "OpentrackerGUI: " << this->GetClassName ( ) << "\n";
...
//---------------------------------------------------------------------------
void vtkOpentrackerGUI::RemoveGUIObservers ( )
{
    // Fill in
    if (this->LoadConfigButton)
    {
...
//---------------------------------------------------------------------------
void vtkOpentrackerGUI::AddGUIObservers ( )
{
...
//---------------------------------------------------------------------------
void vtkOpentrackerGUI::ProcessGUIEvents ( vtkObject *caller,
        unsigned long event, void *callData )
{
    ...
    {
        vtkOpentrackerLogic *igtLogic = this->Logic;
...
//---------------------------------------------------------------------------
void vtkOpentrackerGUI::ProcessLogicEvents ( vtkObject *caller,
        unsigned long event, void *callData )
{
    // Fill in
}
...
//---------------------------------------------------------------------------
void vtkOpentrackerGUI::ProcessMRMLEvents ( vtkObject *caller,
        unsigned long event, void *callData )
{
    // Fill in
}
...
void vtkOpentrackerGUI::ProcessTimerEvents ()
{
    if (! this->StopTimer)
    {
...
void vtkOpentrackerGUI::MakeLocator()
{
...
void vtkOpentrackerGUI::UpdateSliceDisplay(float px, float py, float pz)
{
...
void vtkOpentrackerGUI::UpdateLocator()
{
    vtkOpentrackerLogic *igtLogic = this->Logic;
...
//---------------------------------------------------------------------------
void vtkOpentrackerGUI::BuildGUI ( )
{

    vtkSlicerApplication *app = (vtkSlicerApplication *)this->GetApplication();
    // Define your help text here.
    const char *help = "The **Opentracker Module** connects Slicer to a SPLOT server that feeds a real-time stream of coordinates from a tracking device. This is useful in the operating room to allow the surgeons to navigate the preloaded volume(s) by pointing to a location with a probe that is tracked."; 

    // ---
    // MODULE GUI FRAME 
    // create a page
    this->UIPanel->AddPage ( "Opentracker", "Opentracker", NULL );

    vtkKWWidget *page = this->UIPanel->GetPageWidget ( "Opentracker" );


    // ----------------------------------------------------------------
    // HELP FRAME            
    // ----------------------------------------------------------------
    vtkSlicerModuleCollapsibleFrame *OpentrackerHelpFrame = vtkSlicerModuleCollapsibleFrame::New ( );
    OpentrackerHelpFrame->SetParent ( page );
    OpentrackerHelpFrame->Create ( );
    OpentrackerHelpFrame->CollapseFrame ( );
    OpentrackerHelpFrame->SetLabelText ("Help");
    app->Script ( "pack %s -side top -anchor nw -fill x -padx 2 -pady 2 -in %s",
            OpentrackerHelpFrame->GetWidgetName(), page->GetWidgetName());

    // configure the parent classes help text widget
    this->HelpText->SetParent ( OpentrackerHelpFrame->GetFrame() );
...
void vtkOpentrackerGUI::BuildGUIForRegistrationFrame ()
{
    vtkSlicerApplication *app = (vtkSlicerApplication *)this->GetApplication();
    vtkKWWidget *page = this->UIPanel->GetPageWidget ( "Opentracker" );
...
void vtkOpentrackerGUI::BuildGUIForServerFrame ()
{
    vtkSlicerApplication *app = (vtkSlicerApplication *)this->GetApplication();
    vtkKWWidget *page = this->UIPanel->GetPageWidget ( "Opentracker" );
...
void vtkOpentrackerGUI::BuildGUIForTrackingFrame ()
{
    vtkSlicerApplication *app = (vtkSlicerApplication *)this->GetApplication();
    vtkKWWidget *page = this->UIPanel->GetPageWidget ( "Opentracker" );
...
void vtkOpentrackerGUI::BuildGUIForHandPieceFrame ()
{
    vtkSlicerApplication *app = (vtkSlicerApplication *)this->GetApplication();
    vtkKWWidget *page = this->UIPanel->GetPageWidget ( "Opentracker" );
