...
void qSlicerModelsModuleWidget::setup()
{
  ...
  d->DisplayClassComboBox->addItem("Lines");
  d->DisplayClassComboBox->addItem("Tubes");
  d->DisplayClassComboBox->addItem("Glyphs");
  d->FiberDisplayLabel->setVisible(false);
  d->DisplayClassComboBox->setVisible(false);
  ...
  connect( d->DisplayClassComboBox, SIGNAL(currentIndexChanged(int)),
...
void qSlicerModelsModuleWidget::updateWidgetFromSelectionNode()
{
  ...
  std::string displayNode = selectionNode->GetModelHierarchyDisplayNodeClassName("vtkMRMLFiberBundleNode");
  bool include = !displayNode.empty();

  if (include)
    {
    d->HideChildNodeTypes = (QStringList() << "vtkMRMLAnnotationNode");
    d->DisplayClassComboBox->setVisible(true);
    d->FiberDisplayLabel->setVisible(true);
    }
  else
    {
  ...
  d->FiberDisplayClass.fromStdString(displayNode);
  int index = 0;
  if (displayNode == std::string("vtkMRMLFiberBundleTubeDisplayNode"))
    {
    index = 1;
    }
  else if (displayNode == std::string("vtkMRMLFiberBundleGlyphDisplayNode"))
    {
    index = 2;
    }

  d->DisplayClassComboBox->setCurrentIndex(index);
