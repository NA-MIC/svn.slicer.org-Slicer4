...
ntkElasticTransformationCUDA::ntkElasticTransformationCUDA(){
  m_dInput=NULL;
}

ntkElasticTransformationCUDA::~ntkElasticTransformationCUDA(){
  if(m_dInput!=NULL)delete m_dInput;
}

void ntkElasticTransformationCUDA::loadData(ntk3DData* input){
  m_input=input;
  m_inputSize=m_input->getDataSize();
  m_dInput=new ntkCudaDeviceMemory();
  m_dInput->Allocate<unsigned char>(m_inputSize.x*m_inputSize.y*m_inputSize.z);
  m_dInput->copyFromHost(m_input->getBuffer());
}
...
ntk3DData* ntkElasticTransformationCUDA::applyTransformation(ntkDeformationSpline* splineParam, int splineSizeLevel){
  ...
  ntkCudaDeviceMemory *dOutput = new ntkCudaDeviceMemory();
  dOutput->Allocate<unsigned char>(m_inputSize.x*m_inputSize.y*m_inputSize.z);
  ntkCudaDeviceMemory *dSpline = new ntkCudaDeviceMemory();
  dSpline->Allocate<float>(splineSize.x*splineSize.y*splineSize.z*3);

  dSpline->copyFromHost(splineParam->getBuffer());

  CUDAelasticTransformation_doTransformation((unsigned char*)m_dInput->getDeviceBuffer(), (unsigned char*)dOutput->getDeviceBuffer(), (float*)dSpline->getDeviceBuffer(), m_inputSize.x, m_inputSize.y, m_inputSize.z, splineSizeLevel);

  dOutput->copyToHost(output->getBuffer());
...
ntkFloatDimension ntkElasticTransformationCUDA::getNewPositionFromOld(ntkDeformationSpline* splineParam, int splineSizeLevel, ntkFloatDimension oldPosition, float weight=1.0){
  int a,b,c;
...
ntk3DData* ntkElasticTransformationCUDA::getDeformationFieldImage(ntkDeformationSpline* splineParam, int splineSizeLevel, float weight){
  int i,j,k,a,b,c;
  ntk3DData* output=new ntk3DData(m_inputSize);
...
ntkDeformationSpline *ntkElasticTransformationCUDA::getReverseTransformationSpline(ntkDeformationSpline *inputSpline){
  int i,j,k,a,b,c;
  float relPosX, relPosY, relPosZ;
  float functemp;
  ntkBSpline3Function *func = new ntkBSpline3Function();
...
/*
ntkFloatDimension ntkElasticTransformationCUDA::getDeformationFieldVector(ntkDeformationSpline *splineParam, int splineSizeLevel, ntkFloatDimension position){
