...
vtkCxxRevisionMacro(vtkKWMimxViewPropertiesGroup, "$Revision: 1.15 $");
...
void vtkKWMimxViewPropertiesGroup::CreateWidget()
{
  ...
  this->ActorColorButton->SetLabelText("Surface Color");
  ...
  this->LinewidthThumbwheel->GetWidget()->SetRange(0.0, 1.0);
  this->LinewidthThumbwheel->GetWidget()->SetIncrement(0.01);
  this->LinewidthThumbwheel->GetWidget()->SetValue(0.03);
...
void vtkKWMimxViewPropertiesGroup::SetViewProperties()
{
  ...
  else
  {
    ...
    this->ActorColorButton->SetColor(this->ObjectList->GetItem(row)
    ...
    int representation = this->ObjectList->GetItem(row)->GetActor()
...
void vtkKWMimxViewPropertiesGroup::RepresentationCallback(int type)
{
      ...
      this->ElementSizeScale->EnabledOn();
        ...
        {
        this->ObjectList->GetItem(row)->GetActor()
...
void vtkKWMimxViewPropertiesGroup::ElementSetDisplayPropertyCallback( )
{
        ...
        int meshIndex = this->SelectionRow;
  ...
  this->LinewidthThumbwheel->GetWidget()->SetValue(meshActor->GetElementSetOutlineRadius( setName ));
...
void vtkKWMimxViewPropertiesGroup::SetMeshDisplayPropertyCallback( )
{
  ...
  this->ElementSizeScale->GetWidget()->SetValue(meshActor->GetMeshShrinkFactor()*100.0);
        this->LinewidthThumbwheel->GetWidget()->SetValue(meshActor->GetMeshOutlineRadius());
