...
vtkCxxRevisionMacro(vtkKWMimxBBMenuGroup, "$Revision: 1.18 $");
        ...
        if(this->FileBrowserDialog->GetStatus() == vtkKWDialog::StatusOK)
        {
                if(this->FileBrowserDialog->GetFileName())
                {
                        ...
                        if(!callback->GetState())
                        {
                                ...
                                this->DoUndoTree->GetItem(this->DoUndoTree->GetNumberOfItems()-1)->Data =
                                        vtkMimxUnstructuredGridActor::SafeDownCast(
                                        this->BBoxList->GetItem(this->BBoxList->GetNumberOfItems()-1));
                                this->DoUndoTree->GetItem(this->DoUndoTree->GetNumberOfItems()-1)->Child = NULL;
                                this->DoUndoTree->GetItem(this->DoUndoTree->GetNumberOfItems()-1)->Parent = NULL;
...
//              this->SetMenuButtonsEnabled(0);
        ...
        this->DeleteObjectGroup->GetMainFrame()->SetLabelText("Delete Building Block");
        this->GetApplication()->Script("pack %s -side top -anchor nw -expand n -padx 2 -pady 5 -fill x", 
