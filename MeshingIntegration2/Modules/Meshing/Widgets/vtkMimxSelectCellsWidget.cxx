...
vtkCxxRevisionMacro(vtkMimxSelectCellsWidget, "$Revision: 1.8 $");
...
vtkMimxSelectCellsWidget::vtkMimxSelectCellsWidget()
{
 ...
 this->Inputset = NULL;
  ...
  this->CurrentSelectedSubsetActor->SetMapper(mapper);
  ugrid->Delete();
  mapper->Delete();
  //this->CurrentSelectedSubsetMapper = NULL;
        ...
        //{
                ...
                else
                {
                        ...
                        vtkDataSetMapper::SafeDownCast(
                                self->CurrentSelectedSubsetActor->GetMapper())->SetInput(equivalance->GetOutput());
                        Extract->Delete();
                        mapcellspoints->Delete();
                        equivalance->Delete();
                }
            self->CurrentSelectedSubsetActor->GetMapper()->GetInput()->Modified();
                self->RubberBandStyle->GetInteractor()->SetKeyCode('r');
                self->RubberBandStyle->OnChar();
        ...
        vtkUnstructuredGrid *Input, vtkUnstructuredGrid *CurrSelSubset)
{
        this->Inputset = Input;
        vtkDataSetMapper::SafeDownCast(
                this->CurrentSelectedSubsetActor->GetMapper())->SetInput(CurrSelSubset);
        this->CurrentSelectedSubsetActor->GetMapper()->GetInput()->Modified();
}
//-----------------------------------------------------------------------------------------------
...
void vtkMimxSelectCellsWidget::SetInput(vtkUnstructuredGrid *Input)
{
        ...
        CellArray->SetName("Original_Cell_Ids");
        ugrid->GetCellData()->AddArray(CellArray);
        CellArray->Delete();
...
//---------------------------------------------------------------------------------------
vtkUnstructuredGrid* vtkMimxSelectCellsWidget::GetCurrentSelectedSubset()
{
        return vtkUnstructuredGrid::SafeDownCast(
                this->CurrentSelectedSubsetActor->GetMapper()->GetInput());
}
