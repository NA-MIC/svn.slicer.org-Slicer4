...
vtkCxxRevisionMacro(vtkMimxApplyImageBasedMaterialProperties, "$Revision: 1.4 $");
  ...
  vtkInformationVector *outputVector)
{
  ...
  for (i=0; i < numCells; i++)
  {
          if (elset->GetValue(i))
          {
                        ...
                        double intensityvalue = this->GetAverageSubRegionIntensityValue(StartIndex, EndIndex);
        ...
        GetAverageSubRegionIntensityValue(IntegerType StartIndex, IntegerType EndIndex)
{
        ...
        for (i=0; i<3; i++)
        {
                totalpixels *= (EndIndex[i] - StartIndex[i] + 1);
        }
        PixelIntensity = PixelIntensity/totalpixels;
