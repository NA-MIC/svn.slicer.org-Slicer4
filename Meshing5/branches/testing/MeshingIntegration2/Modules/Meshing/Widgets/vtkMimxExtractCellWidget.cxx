...
vtkCxxRevisionMacro(vtkMimxExtractCellWidget, "$Revision: 1.5 $");
...
//  this->ScalingEnabled = 1;
 ...
 /* for (i=0; i<7; i++)
  ...
  this->PickedCellsCompleteGrid = vtkIdList::New();
  
  this->SelectedCellActor = NULL;
  this->SelectedCellMapper = NULL;
}
...
vtkMimxExtractCellWidget::~vtkMimxExtractCellWidget()
{
 ...
 /* this->HexPicker->Delete();
  ...
  this->PickedCellsCompleteGrid->Delete();
  
  if (this->SelectedCellActor) this->SelectedCellActor->Delete();
  if (this->SelectedCellMapper) this->SelectedCellMapper->Delete();
}
...
void vtkMimxExtractCellWidget::SetEnabled(int enabling)
{
  ...
  if(this->SelectedCellActor)
                this->CurrentRenderer->RemoveActor(this->SelectedCellActor);
...
void vtkMimxExtractCellWidget::OnLeftButtonDown()
{
  ...
  if ( path != NULL )
  {
    ...
    vtkIdType PickedCell = this->CellPicker->GetCellId();
        this->PickedCells->Initialize();
        this->PickedCellsCompleteGrid->Initialize();
    if(PickedCell != -1)
    {
                this->PickedCells->SetNumberOfIds(1);
                this->PickedCellsCompleteGrid->SetNumberOfIds(1);
          ...
          vtkIntArray *cellarray = vtkIntArray::SafeDownCast(
                  this->UGrid->GetCellData()->GetArray("Original_Cell_Ids"));
          if(cellarray)
          {
                this->PickedCellsCompleteGrid->SetId(0, cellarray->GetValue(PickedCell));
          }
    ...
    {
      this->CurrentRenderer->RemoveActor(this->SelectedCellActor);
      this->SelectedCellActor->Delete();     
    }
          if(this->SelectedCellMapper)  this->SelectedCellMapper->Delete();
          this->SelectedCellMapper = vtkDataSetMapper::New();
          this->SelectedCellActor = vtkActor::New();
         
          vtkIdList *cellIds = vtkIdList::New();
          this->UGrid->GetCellPoints(PickedCell, cellIds);
          
          vtkUnstructuredGrid *selectedCellGrid = vtkUnstructuredGrid::New();
          selectedCellGrid->SetPoints(this->UGrid->GetPoints());
          selectedCellGrid->InsertNextCell(VTK_HEXAHEDRON, cellIds);
          this->SelectedCellMapper->SetInput(selectedCellGrid);
          this->SelectedCellActor->SetMapper(this->SelectedCellMapper);
          this->SelectedCellActor->GetProperty()->SetColor(0.0,1.0,0.0);
          this->SelectedCellActor->GetProperty()->SetRepresentationToSurface();
          this->CurrentRenderer->AddActor(this->SelectedCellActor);
