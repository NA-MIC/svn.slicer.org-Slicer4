...
runTest(const int argc, const char** argv, char* data, int2* data_int2, unsigned int len)
{
    ...
    CUDA_SAFE_CALL(cudaMemcpy(d_data, data, mem_size,
                              cudaMemcpyHostToDevice) );
    // allocate device memory for int2 version
    int2* d_data_int2;
    CUDA_SAFE_CALL(cudaMalloc((void**) &d_data_int2, mem_size_int2));
    // copy host memory to device
    CUDA_SAFE_CALL(cudaMemcpy(d_data_int2, data_int2, mem_size_int2,
                              cudaMemcpyHostToDevice) );
    ...
    CUDA_SAFE_CALL(cudaMemcpy(data, d_data, mem_size,
                              cudaMemcpyDeviceToHost));
    CUDA_SAFE_CALL(cudaMemcpy(data_int2, d_data_int2, mem_size_int2,
                              cudaMemcpyDeviceToHost));
    ...
    CUDA_SAFE_CALL(cudaFree(d_data_int2));
    free(reference);
    free(reference2);
}
