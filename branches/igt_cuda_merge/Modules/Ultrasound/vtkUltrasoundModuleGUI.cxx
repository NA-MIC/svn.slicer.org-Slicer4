...
{
    if (caller == this->ScannerReader && event == vtkUltrasoundScannerReader::DataUpdatedEvent)
    {
        this->UpdateInput();
    }
    else if (caller == cb_Enabled && event == vtkKWCheckButton::SelectedStateChangedEvent)
    {
        // ENABLE
        if (cb_Enabled->GetSelectedState() == 1)
        {
            ...
            this->ScannerReader->AddObserver(vtkUltrasoundScannerReader::DataUpdatedEvent, (vtkCommand*)this->GUICallbackCommand);
            this->UpdateInput();
        }
        //DISABLE 
...
{
    if (this->cb_Enabled->GetSelectedState() == 1 && this->ScannerReader != NULL)
    {
        this->ScannerReader->GetImageData(this->VolumeNode->GetImageData());
