#include "ntkElasticRegistration.h"

ntkElasticRegistration::ntkElasticRegistration(){
  m_reference=NULL;
  m_splineParam=NULL;
...
void ntkElasticRegistration::loadData(ntk3DData* reference){
  m_reference=reference;
  m_referenceSize=reference->getDataSize();
  m_referenceBuffer=(unsigned char*)m_reference->getBuffer();
}

void ntkElasticRegistration::doRegistration(ntk3DData* target){}//dummy function

ntkDeformationSpline* ntkElasticRegistration::doRegistration(ntk3DData* reference, ntk3DData* target, ntkElasticityMap* emap, int splineSizeLevel=0, ntkDeformationSpline* splineParam=NULL, float step=1.0, float PEweight=1.0){
  int i,j,k,n;
  float maxGradient;
  m_PEweight=PEweight;
  ...
  ntkElasticTransformation *transform=new ntkElasticTransformation();
  ...
  for(j=0;j<m_splineSize.y;j++){
    for(k=0;k<m_splineSize.z;k++){
      ...
      transform=new ntkElasticTransformation();
      transform->loadData(target);
...
float ntkElasticRegistration::calculateSSDGradient(ntkDeformationSpline *splineParam){
  int i,j,k,n; 
  ...
  int tarSize=m_targetSize.x*m_targetSize.y*m_targetSize.z;
  int splSize=m_splineSize.x*m_splineSize.y*m_splineSize.z;
  float *newX=(float*)malloc(tarSize*sizeof(float));
  float *newY=(float*)malloc(tarSize*sizeof(float));
  float *newZ=(float*)malloc(tarSize*sizeof(float));
  float *diff=(float*)malloc(tarSize*sizeof(float));
  ...
  for(d=(int)relPosX-1;d<(int)relPosX+3;d++){
    ...
    for(e=(int)relPosY-1;e<(int)relPosY+3;e++){
      ...
      for(f=(int)relPosZ-1;f<(int)relPosZ+3;f++){
        ...
        index2=f*m_splineSize.x*m_splineSize.y+e*m_splineSize.x+d;
        *(newX+index)+=*(splineParamBuffer+index2)*functemp;
        *(newY+index)+=*(splineParamBuffer+splSize+index2)*functemp;
        *(newZ+index)+=*(splineParamBuffer+2*splSize+index2)*functemp;
      }
    }
  }
  tempValue=m_transform->getInterpolationValue(m_targetPlusSpline, *(newX+index)+1.0, *(newY+index)+1.0,  *(newZ+index)+1.0);
  
  if(tempValue<0)tempValue=0;
  if(tempValue>255)tempValue=255;

  *(diff+index)=tempValue-*(m_referenceBuffer+index);
  if(*(diff+index)==1 || *(diff+index)==-1)*(diff+index)=0;
...
float ntkElasticRegistration::calculateSSDPEGradient(ntkDeformationSpline *splineParam){
  int i,j,k,n; 
...
void ntkElasticRegistration::threadCalculateSSDGradient(float *newX, float *newY, float* newZ, float *diff,  int threadNumber){
  int i,j,k;
  int a,b,c;
  int d,e,f;
...
void ntkElasticRegistration::threadCalculateSSDPEGradient(float *newX, float *newY, float* newZ, float *diff,  int threadNumber){
  int i,j,k;
  int a,b,c;
  int d,e,f;
  float val1, val2, val3;
...
void* ntkElasticRegistration::startCalculateSSDGradientThread(void *arg){
...
void* ntkElasticRegistration::startCalculateSSDPEGradientThread(void *arg){
  ntkCalculateSSDGradientThreadParam *temp=(ntkCalculateSSDGradientThreadParam*)arg;
  (temp->here)->threadCalculateSSDPEGradient(temp->newX, temp->newY, temp->newZ, temp->diff, temp->threadNumber);
  return 0;
}

float ntkElasticRegistration::calculateSSDGradient(ntk3DData* target, int splineSizeLevel=0, ntkDeformationSpline *splineParam=NULL){
  int j,k;
  m_splineSizeLevel=splineSizeLevel;
  m_target=target;
  m_targetSize=target->getDataSize();
...
float* ntkElasticRegistration::getGradientBuffer(){
  return m_SSDgradient;
}

ntkIntDimension ntkElasticRegistration::getSplineSize(){
  return m_splineSize;
}

float ntkElasticRegistration::calculateSSD(ntk3DData* reference, ntk3DData* target){
  int i,j,k;
  ...
  return value/refDim.x/refDim.y/refDim.z;
}

float ntkElasticRegistration::calculateSSDPE(ntk3DData* reference, ntk3DData* target, ntkDeformationSpline *splineParam){
  int i,j,k;
  float potEnergy=0;
  ntkIntDimension refDim=reference->getDataSize();
  ntkIntDimension tarDim=target->getDataSize();
...
ntkDeformationSpline *ntkElasticRegistration::doSplineParamTransform(ntkDeformationSpline* inputSpline){
...
void ntkElasticRegistration::addRegistrationScheme(int imageResLevel, int splineResLevel, float step=1.0, float PEweight=0.0){
...
void ntkElasticRegistration::resetRegistrationScheme(){
  ntkElasticRegistrationScheme *tempScheme;
...
void ntkElasticRegistration::printRegistrationScheme(){
  ntkElasticRegistrationScheme *tempScheme;
...
bool ntkElasticRegistration::checkRegistrationScheme(){
...
ntkDeformationSpline* ntkElasticRegistration::executeRegistrationSchemes(ntk3DData* reference, ntk3DData *target, ntkDeformationSpline* inputSpline=NULL, ntkElasticityMap* emap=NULL){
  int i;
...
ntk3DData *ntkElasticRegistration::resizeDataSmaller(ntk3DData* input, int imageResLevel){
  int i,j,k;
...
ntkElasticityMap *ntkElasticRegistration::resizeMapSmaller(ntkElasticityMap* input, int imageResLevel){
  int i,j,k;
...
void ntkElasticRegistration::setMinGradientLimit(float value){
