...
#ifndef __vtkOpentrackerGUI_h
#define __vtkOpentrackerGUI_h

#include "vtkOpentrackerWin32Header.h"
...
//
class VTK_OPENTRACKER_EXPORT vtkOpentrackerGUI : public vtkSlicerModuleGUI
{
 public:
    // Description:
    // Usual vtk class functions
    static vtkOpentrackerGUI* New (  );
    vtkTypeRevisionMacro ( vtkOpentrackerGUI, vtkSlicerModuleGUI );
    void PrintSelf (ostream& os, vtkIndent indent );
    
    // Description:
    // Get methods on class members (no Set methods required)
    vtkGetObjectMacro ( Logic, vtkOpentrackerLogic );

    // Description:
    // API for setting VolumeNode, VolumeLogic and
    // for both setting and observing them.
    void SetModuleLogic ( vtkOpentrackerLogic *logic )
        { this->SetLogic ( vtkObjectPointer (&this->Logic), logic ); }
    void SetAndObserveModuleLogic ( vtkOpentrackerLogic *logic )
...
//    vtkKWPushButton *SavePointPairPushButton;
 ...
 private:
    vtkOpentrackerGUI ( const vtkOpentrackerGUI& ); // Not implemented.
    void operator = ( const vtkOpentrackerGUI& ); //Not implemented.
