...
#include "vtkOpentrackerLogic.h"

#include "vtkLandmarkTransform.h"
#include "vtkCylinderSource.h"


vtkCxxRevisionMacro(vtkOpentrackerLogic, "$Revision: 1.9.12.1 $");
vtkStandardNewMacro(vtkOpentrackerLogic);

vtkOpentrackerLogic::vtkOpentrackerLogic()
{
...
void vtkOpentrackerLogic::SetNumberOfPoints(int no)
{
    if (this->TargetLandmarks)
    {
        this->TargetLandmarks->Delete();
...
void vtkOpentrackerLogic::Init(char *configfile)
{
...
void vtkOpentrackerLogic::CloseConnection()
{
#ifdef USE_OPENTRACKER
    context->close();
#endif
...
void vtkOpentrackerLogic::quaternion2xyz(float* orientation, float *normal, float *transnormal) 
{
    float q0, qx, qy, qz;
...
void vtkOpentrackerLogic::PollRealtime()
{
#ifdef USE_OPENTRACKER
...
#ifdef USE_OPENTRACKER
void vtkOpentrackerLogic::callbackF(const Node&, const Event &event, void *data)
{
    float position[3];
    float orientation[4];
    float norm[3];
    ...
    vtkOpentrackerLogic *VOT=(vtkOpentrackerLogic *)data;
...
void vtkOpentrackerLogic::ApplyTransform(float *position, float *norm, float *transnorm)
{
...
void vtkOpentrackerLogic::AddPoint(int id, float t1, float t2, float t3, float s1, float s2, float s3)
{
...
int vtkOpentrackerLogic::DoRegistration()
{

    if (this->TargetLandmarks == NULL || this->SourceLandmarks == NULL)
    {
        vtkErrorMacro(<< "vtkOpentrackerLogic::DoRegistration(): Got NULL pointer.");
        return 1;
    }
    ...
    if (tnp < 2 || snp < 2)
    {
        vtkErrorMacro(<< "vtkOpentrackerLogic::DoRegistration(): Number of points is less than 2 in either TargetLandmarks or SourceLandmarks.");
        return 1;
    }

    if (tnp != snp)
    {
        vtkErrorMacro(<< "vtkOpentrackerLogic::DoRegistration(): TargetLandmarks != SourceLandmarks in terms of number of points.");
        return 1;
    }
...
void vtkOpentrackerLogic::ProcessMRMLEvents(vtkObject * caller, unsigned long event, void * callData)
{
...
void vtkOpentrackerLogic::SetLocatorTransforms()
{
    // Get locator matrix
...
void vtkOpentrackerLogic::UpdateSliceImages()
{
...
void vtkOpentrackerLogic::Normalize(float *a)
{
    float d;
...
// a = b x c
void vtkOpentrackerLogic::Cross(float *a, float *b, float *c)
{
    a[0] = b[1]*c[2] - c[1]*b[2];
    a[1] = c[0]*b[2] - b[0]*c[2];
    a[2] = b[0]*c[1] - c[0]*b[1];
...
void vtkOpentrackerLogic::PrintSelf(ostream& os, vtkIndent indent)
{
    this->vtkObject::PrintSelf(os, indent);

    os << indent << "vtkOpentrackerLogic:             " << this->GetClassName() << "\n";
