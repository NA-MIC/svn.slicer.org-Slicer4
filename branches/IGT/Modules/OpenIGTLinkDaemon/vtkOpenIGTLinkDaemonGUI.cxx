...
Program:   3D Slicer
Module:    $RCSfile: vtkOpenIGTLinkDaemonGUI.cxx,v $
...
#include "vtkOpenIGTLinkDaemonGUI.h"
...
//------------------------------------------------------------------------------
vtkOpenIGTLinkDaemonGUI* vtkOpenIGTLinkDaemonGUI::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkOpenIGTLinkDaemonGUI");
  if(ret)
    {
      return (vtkOpenIGTLinkDaemonGUI*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkOpenIGTLinkDaemonGUI;
}


//----------------------------------------------------------------------------
vtkOpenIGTLinkDaemonGUI::vtkOpenIGTLinkDaemonGUI()
{
...
//----------------------------------------------------------------------------
void vtkOpenIGTLinkDaemonGUI::PrintSelf(ostream& os, vtkIndent indent)
{
  
}

//---------------------------------------------------------------------------
void vtkOpenIGTLinkDaemonGUI::AddGUIObservers ( ) 
{
...
//---------------------------------------------------------------------------
void vtkOpenIGTLinkDaemonGUI::RemoveGUIObservers ( )
{
    // Fill in
...
//---------------------------------------------------------------------------
void vtkOpenIGTLinkDaemonGUI::ProcessGUIEvents ( vtkObject *caller,
                                           unsigned long event,
                                           void *callData ) 
{
  ...
  if ( s == this->ConductanceScale && event == vtkKWScale::ScaleValueChangedEvent ) {
    this->Logic->GetOpenIGTLinkDaemonNode()->SetConductance(this->ConductanceScale->GetValue());
  }
  else if (s == this->TimeStepScale && event == vtkKWScale::ScaleValueChangedEvent ) {
    this->Logic->GetOpenIGTLinkDaemonNode()->SetTimeStep(this->TimeStepScale->GetValue());
  }
    ...
    this->Logic->GetOpenIGTLinkDaemonNode()->SetInputVolumeRef(this->VolumeSelector->GetSelected()->GetID());
  }
...
//---------------------------------------------------------------------------
void vtkOpenIGTLinkDaemonGUI::ProcessMrmlEvents ( vtkObject *caller,
                                            unsigned long event,
                                            void *callData ) 
{
  /**  
  vtkMRMLOpenIGTLinkDaemonNode* node = dynamic_cast<vtkMRMLOpenIGTLinkDaemonNode *> (this->ApplicationLogic->GetMRMLScene()->GetNextNodeByClass("vtkMRMLOpenIGTLinkDaemonNode"));

  if (node) {
    this->SetOpenIGTLinkDaemonNode(node);
  }
  **/
}
...
//---------------------------------------------------------------------------
void vtkOpenIGTLinkDaemonGUI::BuildGUI ( ) 
{
  vtkSlicerApplication *app = (vtkSlicerApplication *)this->GetApplication();
    
  this->UIPanel->AddPage ( "OpenIGTLinkDaemon", "OpenIGTLinkDaemon", NULL );
  // ---
  // MODULE GUI FRAME 
  ...
  helpFrame->SetParent ( this->UIPanel->GetPageWidget ( "OpenIGTLinkDaemon" ) );
  helpFrame->Create ( );
  helpFrame->CollapseFrame ( );
  helpFrame->SetLabelText ("Help");
  app->Script ( "pack %s -side top -anchor nw -fill x -padx 2 -pady 2 -in %s",
                helpFrame->GetWidgetName(), this->UIPanel->GetPageWidget("OpenIGTLinkDaemon")->GetWidgetName());

  vtkKWFrameWithLabel *moduleFrame = vtkKWFrameWithLabel::New ( );
  moduleFrame->SetParent ( this->UIPanel->GetPageWidget ( "OpenIGTLinkDaemon" ) );
  moduleFrame->Create ( );
  moduleFrame->SetLabelText ("OpenIGTLinkDaemon");
  moduleFrame->ExpandFrame ( );
  app->Script ( "pack %s -side top -anchor nw -fill x -padx 2 -pady 2 -in %s",
                moduleFrame->GetWidgetName(), this->UIPanel->GetPageWidget("OpenIGTLinkDaemon")->GetWidgetName());
