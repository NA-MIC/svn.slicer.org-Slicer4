...
vtkCxxRevisionMacro(vtkNeurosurgeryLogic, "$Revision: 1.9.12.1 $");
vtkStandardNewMacro(vtkNeurosurgeryLogic);

vtkNeurosurgeryLogic::vtkNeurosurgeryLogic()
{
...
void vtkNeurosurgeryLogic::SetNumberOfPoints(int no)
{
    if (this->TargetLandmarks)
    {
        this->TargetLandmarks->Delete();
...
void vtkNeurosurgeryLogic::Init(char *configfile)
{
...
void vtkNeurosurgeryLogic::CloseConnection()
{
#ifdef USE_OPENTRACKER
    context->close();
#endif
...
void vtkNeurosurgeryLogic::quaternion2xyz(float* orientation, float *normal, float *transnormal) 
{
    float q0, qx, qy, qz;
...
void vtkNeurosurgeryLogic::PollRealtime()
{
#ifdef USE_OPENTRACKER
...
#ifdef USE_OPENTRACKER
void vtkNeurosurgeryLogic::callbackF(const Node&, const Event &event, void *data)
{
    float position[3];
    float orientation[4];
    float norm[3];
    ...
    vtkNeurosurgeryLogic *VOT=(vtkNeurosurgeryLogic *)data;
...
void vtkNeurosurgeryLogic::ApplyTransform(float *position, float *norm, float *transnorm)
{
...
void vtkNeurosurgeryLogic::AddPoint(int id, float t1, float t2, float t3, float s1, float s2, float s3)
{
...
int vtkNeurosurgeryLogic::DoRegistration()
{

    if (this->TargetLandmarks == NULL || this->SourceLandmarks == NULL)
    {
        vtkErrorMacro(<< "vtkNeurosurgeryLogic::DoRegistration(): Got NULL pointer.");
        return 1;
    }
    ...
    if (tnp < 2 || snp < 2)
    {
        vtkErrorMacro(<< "vtkNeurosurgeryLogic::DoRegistration(): Number of points is less than 2 in either TargetLandmarks or SourceLandmarks.");
        return 1;
    }

    if (tnp != snp)
    {
        vtkErrorMacro(<< "vtkNeurosurgeryLogic::DoRegistration(): TargetLandmarks != SourceLandmarks in terms of number of points.");
        return 1;
    }
...
void vtkNeurosurgeryLogic::ProcessMRMLEvents(vtkObject * caller, unsigned long event, void * callData)
{
...
void vtkNeurosurgeryLogic::SetLocatorTransforms()
{
    // Get locator matrix
...
void vtkNeurosurgeryLogic::UpdateSliceImages()
{
...
void vtkNeurosurgeryLogic::Normalize(float *a)
{
    float d;
...
// a = b x c
void vtkNeurosurgeryLogic::Cross(float *a, float *b, float *c)
{
    a[0] = b[1]*c[2] - c[1]*b[2];
    a[1] = c[0]*b[2] - b[0]*c[2];
    a[2] = b[0]*c[1] - c[0]*b[1];
...
void vtkNeurosurgeryLogic::PrintSelf(ostream& os, vtkIndent indent)
{
    this->vtkObject::PrintSelf(os, indent);

    os << indent << "vtkNeurosurgeryLogic:             " << this->GetClassName() << "\n";
